...
class ApiKeyAuthentication
  ...
  def valid_api_key?(key)
    ...
    # Create a fresh database connection to ensure we're reading the right data
    db = SQLite3::Database.new('database.sqlite3')
    db.results_as_hash = true
    
    # Clean the key and escape for security
    cleaned_key = key.strip.gsub("'", "''")  # SQLite escaping for single quotes
    
    # Use direct query since parameterized queries seem to have issues in middleware context
    result = db.execute("SELECT COUNT(*) as count FROM api_keys WHERE key = '#{cleaned_key}' AND active = 1")
